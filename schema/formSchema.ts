...
// Stage 6
// -----------------------
export const sixthStage = z.object({
...
export const fullFormSchema = firstStage
  ...
  .refine(
    ...
    { path: ["ifOthers"], message: "Kindly specify" }
  )
  .refine(
    (data) => {
  ...
  .superRefine((data, ctx) => {
    if (data.maritalStatus === "Married") {
      if (!data.titleSpouse) {
        ctx.addIssue({ code: "custom", path: ["titleSpouse"], message: "Spouse title is required" });
      }
      if (!data.SurnameSpouse) {
        ctx.addIssue({ code: "custom", path: ["SurnameSpouse"], message: "Spouse surname is required" });
      }
      if (!data.firstNameSpouse) {
        ctx.addIssue({ code: "custom", path: ["firstNameSpouse"], message: "Spouse first name is required" });
      }
      if (!data.spouseAddress) {
        ctx.addIssue({ code: "custom", path: ["spouseAddress"], message: "Spouse address is required" });
      }
      if (!data.spousePhoneNo || !/^\d+$/.test(data.spousePhoneNo)) {
        ctx.addIssue({ code: "custom", path: ["spousePhoneNo"], message: "Spouse phone number must be numbers only" });
      }
    }
  });
